# Poland stock market API

Real-time Poland stock market API with WebSocket, OHLCV, and snapshot endpoints for GPW.

## Build With Poland Market Data

Kun Data provides a token-authenticated API stack for developers who need Poland market data in products that must load fast, stream live prices, and support charting or screening workflows.

This repository is focused on Poland coverage with support for GPW and is designed for teams building broker dashboards, watchlists, quant tools, stock screeners, mobile apps, and market overview pages.

## Why Developers Search For This

Developers usually need one or more of these:

- Real-time Poland stock market API
...
`Poland stock market API | GPW API | Warsaw Stock Exchange API | Poland stock data API`
...
### Does this support real-time Poland market data?
...
If you need Poland market data coverage for GPW, start with the product page and documentation:
